...
softTimeoutDurationEarly = 0.8;    % soft timeout limit for early trials
softTimeoutDurationLate = 0.6;     % soft timeout limit for later trials
...
numDistractType = 2;        % Three types of distractor present trial (high, low)
numDistractTrialsPerBlock = 10;     % 10 - UPDATED 25/01/16 
...
exptTrialsBeforeBreak = 3 * exptTrialsPerBlock;     % 3 * exptTrialsPerBlock = 72

exptTrialsBeforeCalibration = 7 * exptTrialsPerBlock;  % 7 * exptTrialsPerBlock

pracTrials = 8;    % 8
exptTrials = 32 * exptTrialsPerBlock;    % 32 * exptTrialsPerBlock = 768
...
winMultiplier(2) = smallMultiplier;     % Low-val distractors
winMultiplier(3) = bigMultiplier;         % no distractor, big win
...
if exptPhase == 0
    ...
    distractArray = [ones(1,numDistractTrialsPerBlock) ones(1,numDistractTrialsPerBlock)*2 ones(1,numAbsentTrialsPerBlock)*3 ones(1,numAbsentTrialsPerBlock)*4];
...
for trial = 1 : numTrials
    ...
    if trialEnd == 2
        ...
        if exptPhase == 1       % If this is NOT practice
            ...
            elseif omissionTrial == 1
                
                if instrCondition == 1
                    fbStr = ['+', num2str(trialPay), ' ', centCents];
                else
                    fbStr = ['+', num2str(trialPay), ' ', centCents,'\n\nYou looked at the circle'];
                end
    ...
    if exptPhase == 0
        ...
        EGdatafilename = ['EyeData\P', num2str(p_number), '\GazeDataP', num2str(p_number),'S', num2str(exptSession), 'T', num2str(trial), '.mat'];
