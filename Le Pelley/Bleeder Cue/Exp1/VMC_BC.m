...
instrCondition = 1;     % Set this so all participants get no instruction about omission
...
while inputError == 1
    ...
    datafilename = ['BehavData\VMC_BC_dataP', num2str(p_number)];
    
    if exist([datafilename, '.mat'], 'file') == 2
        disp(['Data for participant ', num2str(p_number),' session ', num2str(exptSession),' already exist'])
        inputError = 1;
    end
...
orange = [202 103 19];
green = [52 150 75];
blue = [26 134 247]; %[87 87 255];
pink = [205 94 138]; %197 115 175 %226 111 196
yellow = [255 255 0];
Screen('FillRect',MainWindow, black);

distract_col = zeros(5,6);

distract_col(5,:) = [yellow gray];       % Practice colour
switch colBalance
    case 1
        distract_col(1,:) = [orange blue];      % High-value distractor colour, second colour is the irrelevant distractor
        distract_col(2,:) = [green pink];      % Low-value distractor colour, second colour is the irrelevant distractor
        colourName = char('ORANGE','BLUE','GREEN','PINK'); 
    case 2
        distract_col(1,:) = [blue orange];
        distract_col(2,:) = [pink green];
        colourName = char('BLUE','ORANGE','PINK','GREEN'); 
    case 3
        distract_col(1,:) = [green pink];
        distract_col(2,:) = [orange blue];
        colourName = char('GREEN','PINK','ORANGE','BLUE'); 
    case 4
        distract_col(1,:) = [pink green];
        distract_col(2,:) = [blue orange];
        colourName = char('PINK','GREEN','BLUE','ORANGE');       
end
distract_col(3,:) = [gray gray];
distract_col(4,:) = [gray gray];
...
sessionBonus = sessionPoints / 130;   % convert points into cents at rate of 13 000 points = $1. Updated 13/5.
